...
for noistr in nois:
    ...
    for sizestr in sizs:
        # database from find_all_thresh_vary.m
        fname = 'output/thrs_glob_all_'+sizestr+'_'+noistr+'.txt'
        db = pd.read_csv(fname, delim_whitespace=True)  
        
        txtf = 'output/comp_segment_glob_'+zsamptxt+noistr+'_'+sizestr+crstr+'_hor2D.txt'
        ...
        #    plt.show()
            ...
            while ni<=numnois:
                ...
                # for the given noise level
                thrd = db[db['noise']==ni].values
                # noise iso li mean otsu triangle yen gauss 
                thrd = thrd[0]
                thrs = [thrd[1], thrd[2], thrd[3], thrd[4], thrd[5], thrd[6], thrd[7]]
